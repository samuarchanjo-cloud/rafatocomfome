...
function Orders({ orders, loading, error }) {
  return <div className="admin-section"><div className="admin-section-title"><h2>Pedidos</h2><span>Últimos 100 pedidos</span></div>{loading&&<p className="empty">Carregando pedidos...</p>}{error&&<div className="admin-warning">{error}</div>}<div className="orders-list">{orders.map((order)=><article key={order.id} className="admin-card"><div className="order-heading"><strong>#{String(order.id).slice(0,8)} · {order.customer_name}</strong><span>{money(order.total)}</span></div><small>{new Date(order.created_at).toLocaleString("pt-BR")} · {order.delivery_type} · {order.delivery_mode === "uber" ? "Uber solicitado pelo cliente" : "Entrega própria"} · {order.payment_method}</small><p>{(order.order_items||[]).map((item)=>`${item.quantity}x ${item.product_name}`).join(", ")}</p>{order.delivery_type==="entrega"&&<small>{order.distance_km!=null?`Distância: ${Number(order.distance_km).toFixed(2)} km · `:""}{order.delivery_mode === "uber" ? "Taxa da loja: R$ 0,00" : `Entrega: ${money(order.delivery_fee)}`}</small>}</article>)}</div>{!loading&&!error&&orders.length===0&&<p className="empty">Nenhum pedido salvo no Supabase.</p>}</div>;
}
