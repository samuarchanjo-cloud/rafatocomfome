...
-- Metadados opcionais para manter compatibilidade com pedidos históricos.
...
begin
  if not exists (
    ...
    where conrelid = 'public.orders'::regclass
      and conname = 'orders_location_source_allowed'
  ) then
    alter table public.orders
      add constraint orders_location_source_allowed
      check (location_source is null or location_source in ('nominatim_exact', 'device_gps'));
  end if;

  if not exists (
    select 1 from pg_constraint
    where conrelid = 'public.orders'::regclass
      and conname = 'orders_location_accuracy_valid'
  ) then
    alter table public.orders
      add constraint orders_location_accuracy_valid
      check (
        (location_source is null and location_accuracy_m is null)
        or (location_source = 'nominatim_exact' and location_accuracy_m is null)
        or (
...
begin
  ...
  if v_delivery_type = 'entrega' then
    ...
    if v_location_source is null or v_location_source not in ('nominatim_exact', 'device_gps') then
    ...
    elsif nullif(trim(coalesce(p_order->>'location_accuracy_m', '')), '') is not null then
      raise exception 'INVALID_LOCATION_ACCURACY';
    end if;
  ...
  insert into public.orders (
    ...
    v_location_source, v_location_accuracy_m,
