...
alter table public.orders
  add column if not exists location_uncertainty_m numeric(10,2);
...
alter table public.orders
  ...
  check (location_source is null or location_source in ('nominatim_exact', 'device_gps', 'address_consensus'));
...
alter table public.orders
  ...
  check (
    ...
    or (location_source = 'address_consensus' and location_accuracy_m is null)
    or (
...
alter table public.orders
  ...
  check (
    ...
    or (location_source in ('nominatim_exact', 'device_gps') and location_uncertainty_m is null)
...
begin
  ...
  if v_delivery_type = 'entrega' then
    ...
    if v_location_source is null
       or v_location_source not in ('nominatim_exact', 'device_gps', 'address_consensus') then
