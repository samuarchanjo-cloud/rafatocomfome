...
create table if not exists public.delivery_postal_zones (
  id uuid primary key default gen_random_uuid(),
  postal_code text not null unique check (postal_code ~ '^[0-9]{8}$'),
  label text,
...
create index if not exists delivery_postal_zones_active_postal_code_idx
  on public.delivery_postal_zones(postal_code)
  where active;

drop trigger if exists set_delivery_postal_zones_updated_at on public.delivery_postal_zones;
create trigger set_delivery_postal_zones_updated_at
  before update on public.delivery_postal_zones
  for each row execute function public.set_updated_at();

alter table public.delivery_postal_zones enable row level security;

drop policy if exists delivery_postal_zones_admin_select on public.delivery_postal_zones;
drop policy if exists delivery_postal_zones_admin_insert on public.delivery_postal_zones;
drop policy if exists delivery_postal_zones_admin_update on public.delivery_postal_zones;
drop policy if exists delivery_postal_zones_admin_delete on public.delivery_postal_zones;

create policy delivery_postal_zones_admin_select on public.delivery_postal_zones
  for select to authenticated using (public.is_admin());
create policy delivery_postal_zones_admin_insert on public.delivery_postal_zones
  for insert to authenticated with check (public.is_admin());
create policy delivery_postal_zones_admin_update on public.delivery_postal_zones
  for update to authenticated using (public.is_admin()) with check (public.is_admin());
create policy delivery_postal_zones_admin_delete on public.delivery_postal_zones
  for delete to authenticated using (public.is_admin());

revoke all on table public.delivery_postal_zones from anon;
grant select, insert, update, delete on table public.delivery_postal_zones to authenticated;
...
as $$
  select zone.postal_code, zone.delivery_fee
  from public.delivery_postal_zones zone
  where zone.active
    and zone.postal_code = regexp_replace(coalesce(p_postal_code, ''), '[^0-9]', '', 'g')
  limit 1;
$$;
...
alter table public.orders
  ...
  check (
    ...
    or location_source in ('nominatim_exact', 'google_exact', 'device_gps', 'map_pin', 'address_consensus', 'postal_zone')
  );
...
alter table public.orders
  ...
  check (
    ...
    or (location_source in ('nominatim_exact', 'google_exact', 'map_pin', 'address_consensus', 'postal_zone') and location_accuracy_m is null)
    or (
...
alter table public.orders
  ...
  check (
    ...
    or (location_source in ('nominatim_exact', 'google_exact', 'device_gps', 'map_pin', 'postal_zone') and location_uncertainty_m is null)
...
declare
  ...
  v_postal_zone public.delivery_postal_zones%rowtype;
  v_item jsonb;
...
begin
  ...
  if v_delivery_type = 'entrega' then
    ...
    if v_location_source = 'postal_zone' then
      ...
      select zone.* into v_postal_zone
        from public.delivery_postal_zones zone
        where zone.postal_code = v_postal_code and zone.active
        limit 1;
      ...
      v_raw_distance := null;
      v_distance := null;
      v_delivery_fee := v_postal_zone.delivery_fee;
    else
      begin
